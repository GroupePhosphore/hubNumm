...
class SalesforceClient
{
    ...
    {
        return $this->makeCallToNummApi('GET', '/services/data/v57.0/query/?q=' . urlencode($query));
    }
    ...
    {
        return $this->makeCallToNummApi('GET', $nextPageURL);
    }

    /**
     * Make an API to the Numm app instance
     *
     * @param string $method
     * @return Object
     */
    private function makeCallToNummApi(string $method, string $url, array $body = []): Object
    {
        if (!isset($this->authToken) || !isset($this->instanceUrl)) {
            $this->setSecurityTokenAndInstanceUrl();
        }

        if (!isset($this->nummClient)) {
            $this->nummClient = new Client(['base_uri' => $this->instanceUrl]);
        }
        
        try {

            $apiCall = $this->nummClient->request(
                $method,
                $url,
                [
